...
func NewImportService(config *config.Config, customerRepo repo.CustomerRepository, patientRepo repo.PatientBackend, resolver session.PriorityResolver, catalog discovery.Discoverer) *ImportService {
...
func (svc *ImportService) ImportSession(ctx context.Context, stream *connect.BidiStream[customerv1.ImportSessionRequest, customerv1.ImportSessionResponse]) error {
	...
	session := session.NewImportSession(svc.config.Country, stream, svc.customerRepository, svc.patientRepository, svc.resolver, svc.catalog)
